...
def state_generator(state, action):
    ...
    # 사망 프로세스
    if branch[:2] in ['51', '61'] and action == 0 or branch[:2] == ['52', '62'] and action == 1: return torch.tensor([0., 0., 0., 0., 1.], device='cuda'), True
    # 생존 프로세스
    else:
        if branch[0] == '6':
            if len(branch) > 2: branch = branch[2:]
            else: return torch.tensor([random.choice([11., 12.]), player, 0., 0., 0.], device='cuda'), False

        add = ''
        for _ in range(len(branch) >> 1): add += '10'
...
if __name__ == '__main__':
    ...
    # 에이전트 생성
    env = RL.Environment()
    agent = RL.Agent(obj.epsilon, obj.epsilon_discount, obj.learning_rate, obj.node, True, obj.batch_size)
    ...
    while True:
        ...
        # TD(N)
        agent.Batch.append([state.detach(), action, reward, next_state.detach()])
        if done:
            for i in range(len(agent.Batch)):
                State, V_value, Action, Reward, Advantage, Q_value = agent.Variable_ready_for_TD_N_Parallel()
